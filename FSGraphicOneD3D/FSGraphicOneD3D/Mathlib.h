...
#define NEAR_PLANE 0.1f
	...
	inline float DegreeToRadians(float _degree) { return _degree * (PI / 180.0f); }

	inline float RadiansToDegree(float _radians) { return _radians * (180.0f / PI); }
	...
	template<typename T>
	inline T Lerp_(const T& _A, const T& _B, float _ratio) { return _A * (_B - _A) * _ratio; }

	template<typename T>
	inline T RandInRange(T _min, T _max) { return _min + (rand() % (_max - _min + 1)); }
	
	inline int Convert2Dto1D(const unsigned int _x, const unsigned int _y, const unsigned int _width) { return _y*_width + _x; }
